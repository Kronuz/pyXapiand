...
def _database_command(databases_pool, cmd, db, args, data='.', log=logging):
    unknown = False
    start = time.time()
...
def _database_commit(databases_pool, to_commit, commit_lock, timeouts, force=False, data='.', log=logging):
    if not to_commit:
        return
